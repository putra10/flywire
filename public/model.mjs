...
export function decodeMatrix(buffer, metadata) {
  ...
  if (metadata.schemaVersion !== 1 || metadata.neurons.length !== n ||
      metadata.matrix.dtype !== 'float32' || metadata.matrix.endian !== 'little' ||
      metadata.matrix.orientation !== 'source,target' || buffer.byteLength !== n * n * 4) {
    throw new Error('The connectome files do not match. Re-export both data files.');
  }
  const view = new DataView(buffer);
  const matrix = new Float32Array(n * n);
  let edges = 0, synapses = 0;
...
export function normalizeWeights(counts, neurons) {
  const n = neurons.length, weights = new Float32Array(n * n);
...
export function shuffleWeights(matrix, n, seed) {
  const random = randomGenerator(seed), shuffled = matrix.slice();
  for (let source = 0; source < n; source++) {
