...
const simulate = (c, seconds) => {
  for (let i = 0; i < Math.round(seconds / STEP); i++) c.step();
};
...
test('shipped measured data match their hash, counts, and string root IDs', () => {
  ...
  for (const n of metadata.neurons) {
    assert.match(n.id, /^\d{18}$/);
    assert.ok(Number.isFinite(n.angle));
  }
});
...
test('shuffle preserves every source row weight multiset and exact edge count', () => {
  for (const seed of [1, 42, 100]) {
    ...
    for (let source = 0; source < metadata.neuronCount; source++) {
      const from = source * metadata.neuronCount,
        to = from + metadata.neuronCount;
...
test('real wiring holds a localized stationary patch with no continuing cue', () => {
  for (const phase of [0, 1, 2, 3, 4, 5]) {
    ...
    c.reset(phase);
    simulate(c, 20);
    const first = c.readout();
    simulate(c, 10);
    const last = c.readout();
    assert.ok(last.coherence > 0.6, `phase ${phase}: ${last.coherence}`);
    assert.ok(last.peak > 0.7);
    assert.ok(Math.abs(delta(last.angle, first.angle)) < 0.06, 'settled heading is stable');
...
test('default and independently seeded shuffled graphs lose spatial coherence', () => {
  ...
  for (let seed = 1; seed <= 20; seed++) {
    const c = new Circuit(metadata, counts, seed);
    simulate(c, 8);
    const before = c.rates.slice();
    c.setMode('shuffled');
    assert.deepEqual(c.rates, before);
    simulate(c, 12);
    scores.push(c.readout().coherence);
    assert.equal(c.edges.length, metadata.edgeCount);
    assert.ok(c.rates.every((r) => Number.isFinite(r) && r >= 0 && r <= 1));
  }
  scores.sort((a, b) => a - b);
  assert.ok(scores[10] < 0.3, `median shuffled coherence ${scores[10]}`);
  const c = new Circuit(metadata, counts, 42);
  simulate(c, 8);
  c.setMode('shuffled');
  simulate(c, 12);
  assert.ok(c.readout().coherence < 0.25);
});
...
test('left and right cues move the patch around a complete ring, then it persists', () => {
  for (const velocity of [-45, 45]) {
    const c = new Circuit(metadata, counts);
    simulate(c, 10);
    c.cueAngle = c.readout().angle;
    c.velocity = velocity;
    let previous = c.readout().angle,
      travel = 0;
    for (let second = 0; second < 8; second++) {
      simulate(c, 1);
      const r = c.readout();
      travel += delta(r.angle, previous);
      previous = r.angle;
      assert.ok(r.coherence > 0.7);
    }
    assert.ok(travel * Math.sign(velocity) > 5.5, `turn travel ${travel}`);
    c.velocity = 0;
    simulate(c, 10);
    assert.ok(c.readout().coherence > 0.6);
  }
});
