...
async function boot({ webgl = true, data = true } = {}) {
  ...
  for (const tag of html.matchAll(/<[^>]+\bid="([^"]+)"[^>]*>/g)) {
    ...
    elements.set(tag[1], {
      ...
      click() { assert.equal(this.disabled, false, 'button must be enabled'); return events.get('click')?.(); },
      getBoundingClientRect() { return this.hidden ? { width: 0, height: 0 } : { width: 800, height: 600 }; },
  ...
  class FakeScene {
    constructor() { if (!webgl) throw new Error('WebGL unavailable'); scene = this; }
    setEdges() {}
    setView(view) { this.view = view; }
    resize(width, height) { this.width = width; this.height = height; }
    setAnatomy(anatomy) { this.anatomy = anatomy; }
  }
  const sandbox = {
    Circuit, decodeMatrix, STEP, TAU, Scene3D: FakeScene,
    anatomyFromSkeletonBuffers(neurons, buffers) {
      assert.equal(buffers.length, neurons.length);
      return { loaded: true };
    },
    document: { getElementById: id => elements.get(id) ?? null, body: { classList: { toggle() {} } }, addEventListener() {} },
    matchMedia: () => ({ matches: false }), devicePixelRatio: 1, crypto: webcrypto,
    ResizeObserver: class { constructor(fn) { this.callback = fn; } observe(element) { observed = element; } },
    requestAnimationFrame() {}, console: { error: e => errors.push(e), warn() {} },
    async fetch(url) {
      requests.push(url);
      return {
        ok: data, status: data ? 200 : 404,
        async json() { return meta; },
  ...
  await runInNewContext(source.replace('requestAnimationFrame(frame);init();', 'requestAnimationFrame(frame);globalThis.ready=init();'), sandbox);
  await sandbox.ready;
...
test('startup enables all views and switches from 3D to 2D and real anatomy', async () => {
  ...
  assert.equal(scene.anatomy.loaded, true);
  assert.equal(el.get('anatomy-filter').hidden, false);
  assert.equal(el.get('anatomy-status').hidden, true);
  assert.equal(requests.filter(url => url.startsWith('https://')).length, meta.neuronCount);
});
